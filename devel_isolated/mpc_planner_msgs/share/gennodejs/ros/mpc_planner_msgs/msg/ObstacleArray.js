...
let ObstacleGMM = require('./ObstacleGMM.js');
...
class ObstacleArray {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.obstacles.forEach((val) => {
      bufferOffset = ObstacleGMM.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    for (let i = 0; i < len; ++i) {
      data.obstacles[i] = ObstacleGMM.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    ...
    object.obstacles.forEach((val) => {
      length += ObstacleGMM.getMessageSize(val);
    });
    return length + 4;
  }
  ...
  static Resolve(msg) {
    ...
    if (msg.obstacles !== undefined) {
      ...
      for (let i = 0; i < resolved.obstacles.length; ++i) {
        resolved.obstacles[i] = ObstacleGMM.Resolve(msg.obstacles[i]);
      }
    }
    else {
      resolved.obstacles = []
