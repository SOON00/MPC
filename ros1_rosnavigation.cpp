...
namespace local_planner
{
    ...
    void ROSNavigationPlanner::initializeSubscribersAndPublishers(ros::NodeHandle &nh)
    {
        ...
        guidance_path_sub_ = nh.subscribe("/my_guidance_path", 1,
    &ROSNavigationPlanner::guidancePathCallback, this);

        follow_global_path_pub_ = nh.advertise<std_msgs::Bool>("/follow_global_path", 1);
        ...
        //_costmap_sub = nh.subscribe("/move_base/local_costmap/costmap", 1, &ROSNavigationPlanner::costmapCallback, this);
    }
    void ROSNavigationPlanner::guidancePathCallback(const nav_msgs::Path::ConstPtr& msg)
    {
        _guidance_path = *msg;
    }
    void ROSNavigationPlanner::globalCostmapCallback(const nav_msgs::OccupancyGrid::ConstPtr& msg)
    {
        latest_global_costmap_ = msg;
    }


void ROSNavigationPlanner::costmapCallback(const nav_msgs::OccupancyGrid::ConstPtr& msg)
{
    if (!_planner)
    {
        ROS_WARN("Planner null in costmapCallback");
        return;
    }
    ...
    if (CONFIG["obstacle_radius"] && CONFIG["obstacle_radius"].IsDefined())
        obstacle_radius = CONFIG["obstacle_radius"].as<double>();
    if (obstacle_radius <= 0.0)
        obstacle_radius = 0.1;

    static int dynamic_id = 10000;
    ...
    for (int y = 0; y < height; ++y)
    {
        for (int x = 0; x < width; ++x)
        {
            ...
            double effective_radius = std::max(obstacle_radius, 0.1);
            _data.dynamic_obstacles.emplace_back(
                dynamic_id++,
                Eigen::Vector2d(wx, wy),
                0.0,
                effective_radius
            );

            auto& obs = _data.dynamic_obstacles.back();
            obs.prediction = Prediction(PredictionType::DETERMINISTIC);
            obs.prediction.modes.resize(1);

            for (int i = 0; i < 5; ++i)
            {
                obs.prediction.modes[0].emplace_back(
                    Eigen::Vector2d(wx, wy),
                    0.0,
                    effective_radius,
                    effective_radius
